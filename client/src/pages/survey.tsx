...
export default function Survey() {
  ...
  const onSubmit = (data: SurveyValues) => {
  ...
  const nextStep = async () => {
    const fieldsToValidate = step === 1 
      ? ["age", "gender", "familyHistory", "workInterfere"] as const
      : ["remoteWork", "techCompany", "benefits"] as const;
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                          ...
                          <FormControl>
                            ...
                            >
                              {["Yes", "No", "Not sure"].map((option) => (
                                <FormItem key={option} className="flex items-center space-x-3 space-y-0 p-4 rounded-xl border-2 border-transparent bg-background hover:bg-muted/50 cursor-pointer transition-all shadow-sm has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                                  <FormControl>
                      ...
                      render={({ field }) => (
                        <FormItem className="space-y-3">
                          ...
                          <FormControl>
                            ...
                            >
                              {["Often", "Sometimes", "Rarely", "Never", "Not applicable"].map((option) => (
                                <FormItem key={option} className="flex items-center space-x-3 space-y-0 p-4 rounded-xl border-2 border-transparent bg-background hover:bg-muted/50 cursor-pointer transition-all shadow-sm has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                                  <FormControl>
              ...
              <div className="flex justify-end pt-8 mt-8 border-t border-border/50">
                {step === 1 ? (
                  <Button type="button" size="lg" className="rounded-full h-14 px-8 shadow-md shadow-primary/20 text-lg font-bold" onClick={nextStep} data-testid="button-next">
                    Continue <ArrowRight className="ml-2 w-5 h-5" />
                  </Button>
                ) : (
                  <Button type="submit" size="lg" className="rounded-full h-14 px-8 shadow-lg shadow-primary/30 text-lg font-bold" disabled={isSubmitting} data-testid="button-submit">
                    {isSubmitting ? (
                      <>
